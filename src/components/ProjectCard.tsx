...
export default function ProjectCard({ project, ufHoy }: Props) {
  ...
  const waText = `Hola, me interesa ${project.titulo} (${project.comuna}) desde ${project.desdeUF} UF. ¿Podemos coordinar visita?`;
  ...
  return (
    <>
      <article className="group overflow-hidden rounded-3xl border border-white/70 bg-white/90 shadow-[0_24px_60px_rgba(14,33,73,0.08)] transition hover:-translate-y-0.5 hover:shadow-[0_32px_80px_rgba(14,33,73,0.12)]">
        ...
        <div className="flex flex-col gap-5 p-6">
          <header className="flex items-start justify-between gap-5">
            <div>
              <h3 className="text-lg font-semibold text-brand-navy">{project.titulo}</h3>
              <p className="text-sm text-brand-mute">
                {project.comuna} · {project.tipologias.join(" / ")}
              </p>
            </div>
            <div className="text-right">
              <div className="text-xs uppercase tracking-wide text-brand-mute/70">Desde</div>
          ...
          <div className="rounded-2xl bg-brand-sand/60 p-4 text-sm text-brand-mute">
            <p>
              Revisión legal y financiera incluida. Entregamos comparativa de dividendos y escenarios
              de arriendo en menos de 48 horas.
            </p>
          </div>
          ...
          <footer className="flex flex-wrap items-center gap-3">
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="ml-auto inline-flex items-center gap-2 rounded-full border border-emerald-600/20 bg-emerald-600/10 px-3 py-1.5 text-sm font-medium text-emerald-700 transition hover:bg-emerald-600/15"
              aria-label="WhatsApp"
            >
      ...
      <ProjectQuickView open={open} onClose={() => setOpen(false)} project={project} ufHoy={ufHoy} />
    </>
  );
}
